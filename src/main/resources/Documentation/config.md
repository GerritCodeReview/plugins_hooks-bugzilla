...
If `plugin.bugzilla.enabled` is not specified in the `project.config` file
...
By setting `plugin.bugzilla.enabled` to true in the `project.config` of the
...
setting `plugin.bugzilla.enabled` to `enforced`.

On the project info screen there is a dropdown list for the
`plugin.bugzilla.enabled` parameter which offers the values `true`,
...
To limit the Bugzilla integration to certain branches one or many refs,
ref patterns or regular expressions can be specified as
`plugin.bugzilla.branch`.
